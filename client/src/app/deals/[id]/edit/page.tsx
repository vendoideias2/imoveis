...
export default function EditDealPage({ params }: { params: { id: string } }) {
    ...
    const fetchDeal = async () => {
        try {
            ...
            if (res.ok) {
                ...
                alert('Deal não encontrado');
                router.push('/deals');
            }
        } catch (error) {
            console.error('Error fetching deal:', error);
            alert('Erro ao carregar deal');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                alert('Deal atualizado com sucesso!');
                router.push('/deals');
            } else {
    ...
    if (!deal) {
        return (
            <Layout>
                <div className="text-center py-12">
                    <p className="text-red-600">Deal não encontrado</p>
                </div>
            </Layout>
        );
    }
    ...
    return (
        <Layout>
            <div className="max-w-3xl mx-auto">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold text-gray-800">Editar Deal</h1>
                    <p className="text-gray-600 mt-1">Atualize as informações da proposta comercial</p>
